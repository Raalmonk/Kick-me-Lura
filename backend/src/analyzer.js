...
export async function analyzeReport({ reportCode, rosterText, userToken, lastPullOnly = true }) {
  const rosterRows = parseRoster(rosterText);

  const reportData = await wclGraphql(userToken, REPORT_FIGHTS_AND_ACTORS, { code: reportCode });
  ...
  for (const fight of selectedFights) {
    ...
    const playerInfo = await wclGraphql(userToken, PLAYER_INFO_IN_FIGHT, {
      code: reportCode,
      fightIDs: [fightID]
    });
    ...
    const eventsResp = await wclGraphql(userToken, INTERRUPT_AND_CAST_EVENTS, {
      code: reportCode,
